...
where times are given in the format HH:MM:SS, and ID_number is a string with no 
...
Note: It is guaranteed that the records are consistent. That is, the sign in time must
be earlier than the sign out time for each person, and there are no two persons sign 
in or out at the same moment.
...
int main()
{
    int M;
    scanf("%d", &M);
    struct Info temp, FirstOne, LatestOne;
    FirstOne.time = 86400;
    LatestOne.time = -1;
    for(int i = 0; i < M; i++)
    {
        int hh, mm, ss;
        scanf("%s %d:%d:%d", temp.ID, &hh, &mm, &ss);
        temp.time = hh * 3600 + mm * 60 + ss;
        if(temp.time < FirstOne.time) 
            FirstOne = temp;
        scanf("%d:%d:%d", &hh, &mm, &ss);
        temp.time = hh * 3600 + mm * 60 + ss;    
        if(temp.time > LatestOne.time) 
            LatestOne = temp;
    }
    printf("%s %s\n", FirstOne.ID, LatestOne.ID);
    return 0;
}